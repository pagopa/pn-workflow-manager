...
import static it.pagopa.pn.workflowmanager.exceptions.WorkflowManagerExceptionCodes.ERROR_CODE_WORKFLOWMANAGER_ROUTER_EVENT_TYPE_MISSING;
import static it.pagopa.pn.workflowmanager.exceptions.WorkflowManagerExceptionCodes.ERROR_CODE_WORKFLOWMANAGER_ROUTER_HANDLER_NOT_FOUND;
...
public class EventRouter {
    ...
    public <T> void route(Message<T> message, RoutingConfig config) {
        ...
        if (!StringUtils.hasText(eventType)) {
            throw new PnEventRouterException("Event type must be provided", ERROR_CODE_WORKFLOWMANAGER_ROUTER_EVENT_TYPE_MISSING);
        }
        ...
        EventHandler<T> handler = (EventHandler<T>) handlerRegistry.getHandler(eventType)
                .orElseThrow(() -> new PnEventRouterException("No handler found for event type: " + eventType + " in queue: " + queueName, ERROR_CODE_WORKFLOWMANAGER_ROUTER_HANDLER_NOT_FOUND));

...
class EventRouterTest {
    ...
    void routeThrowsExceptionWhenEventTypeExtractorReturnsNull() {
        ...
        inspectErrorCode(exception);
    }

    @Test
    void routeHandlesMessageOkWithDefaultConfigs() {
        Message<String> message = buildMessageWithHeadersAndPayload();
    ...
    private Message<String> buildMessageWithHeadersAndPayload() {
        Message<String> message = mock(Message.class);
        Map<String, Object> headersMap = new HashMap<>();
        headersMap.put("eventType", "CUSTOM_EVENT");
    ...
    private void inspectErrorCode(PnEventRouterException exception) {
        Assertions.assertEquals(it.pagopa.pn.workflowmanager.exceptions.WorkflowManagerExceptionCodes.ERROR_CODE_WORKFLOWMANAGER_ROUTER_EVENT_TYPE_MISSING, exception.getProblem().getErrors().getFirst().getCode());
    }

...
const MetricCategories = { 
    ...
    "SEND_DIGITAL_MESSAGE": (channel) => `received_${channel}`, 
    "SEND_COURTESY_MESSAGE": (channel) => `received_${channel}`,
...
exports.handleEvent = async (event) => {
    ...
    for (const cdcEvent of cdcEvents) {
        try {
            ...
            if (cdcEvent.eventName !== "INSERT" || !cdcEvent.dynamodb.NewImage) {
                continue;
            }
            ...
            switch (category) {
                ...
                case "PAYMENT": campaignAggregates[campaignId].counters[MetricCategories["PAYMENT"]] = (campaignAggregates[campaignId].counters[MetricCategories["PAYMENT"]] || 0) + 1; break;
                ...
                case "SEND_COURTESY_MESSAGE":
                    let channel = parsedData.details?.channel; // Es: IO, PEC, EMAIL, SMS, ANALOG
                    if (DIGITAL_CHANNELS.includes(channel)) 
                        campaignAggregates[campaignId].counters[ MetricCategories[category](channel)] = (campaignAggregates[campaignId].counters[metricName] || 0) + 1;
                    else {
                ...
                case "REACHED":
                    ...
                    const metricName = `digitalSent_${channel}`;
                    campaignAggregates[campaignId].counters[metricName] = (campaignAggregates[campaignId].counters[metricName] || 0)++;
                    if (ALL_CHANNELS.includes(channel)) {
                        const metricName = `received_${channel}`;
                        campaignAggregates[campaignId].counters[metricName] = (campaignAggregates[campaignId].counters[metricName] || 0)++;
                    }
                break;
                case "PAYMENT":
            ...
            console.error(`Parsing error on record: ${JSON.stringify(record)}. Error:`, err);
    ...
    const updatePromises = Object.keys(campaignAggregates).map();
